...
{#if thread}
	<div class="flex h-full min-h-0 flex-col">
		<div class="flex shrink-0 justify-between border-b bg-white p-3">
			<div class="flex items-center gap-2">
				<div class="text-lg text-black"><ThreadTitle {thread} /></div>
			</div>
			<div class="flex">
		...
		<div class="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto bg-white p-3">
...
{#snippet iconButton({ label, Icon }: { label: string; Icon: Component<IconProps> })}
	<div class="tooltip tooltip-bottom" data-tip={label}>
		<button class="btn p-2 text-black btn-ghost">
			<Icon />
		</button>
	</div>
{/snippet}
