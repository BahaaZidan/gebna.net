...
export const emailAddressRefs = sqliteTable(
	...
	(self) => [
		uniqueIndex("uniq_idx_ownerId_address").on(self.ownerId, self.address),
	],
);
...
export const emailThreads = sqliteTable(
	...
	(self) => [
		index("idx_thread_ownerId_last_message_at").on(
			self.ownerId,
			self.lastMessageAt,
		),
	],
...
export const emailMessages = sqliteTable(
	"emailMessages",
	{
		...
		threadId: text()
			.notNull()
			.references((): AnySQLiteColumn => emailThreads.id, {
				onDelete: "cascade",
			}),
		from: citext().notNull(),
		to: citext().notNull(),
	...
	(self) => [
		...
		uniqueIndex("uniq_idx_message_canonical_message_id").on(
			self.ownerId,
			self.canonicalMessageId,
		),
