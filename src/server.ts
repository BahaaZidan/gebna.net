...
export default {
	...
	async email(envelope) {
		try {
			...
			if (!recipientUser || !recipientUser.ownAddressRef)
				return envelope.setReject("NOT_FOUND");
			...
			const [canonicalMessageId] = extractMessageIdsFromPostalMimeValue(
				...
				if (parsedEnvelope.attachments.length) {
					await tx.insert(dbSchema.emailAttachments).values(
						parsedEnvelope.attachments
							.filter(async (a) => {
								let fileTypeResult = await fileTypeFromBuffer(
									a.content as ArrayBuffer,
								);
								if (!fileTypeResult) return false;
								if (fileTypeResult.mime !== a.mimeType) return false;
								if (
									!ALLOWED_ATTACHMENT_MIME_TYPES.includes(fileTypeResult.mime)
								)
									return false;
								return true;
							})
				...
				try {
					switch (message.body.type) {
						case "infer-address-avatar":
							return await inferAddressAvatar({
								db,
								address: message.body.payload.address,
							});
						default:
							return;
					}
